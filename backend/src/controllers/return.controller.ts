...
export const sellerGetReturnQuote = async (req: Request, res: Response) => {
  try {
    ...
    if (seller?.pickupAddresses && seller.pickupAddresses.length > 0) {
      // Try to find the address that matches courierCartPickupAddressId if available
      const courierCartPickupId = shipmentPlain?.courierCart?.courierCartPickupAddressId
      let selectedAddress: any = null

      if (courierCartPickupId) {
        selectedAddress = seller.pickupAddresses.find(
          (addr: any) => addr.courierCartPickupAddressId === courierCartPickupId,
        )
      }
    ...
    if (!shipmentPlain?.courierCart?.order_id && !shipmentPlain?.courierCart?.order_number) {
      console.error('Shipping provider shipment data not found for return:', {
        returnId: id,
        orderId: orderDoc._id,
        shipmentCourierCart: shipmentPlain?.courierCart,
        hasCourierCart: !!shipmentPlain?.courierCart,
      })
      return res.status(400).json({
        success: false,
        message:
    ...
    try {
      const serviceabilityRequest = {
        ...
        pickup_id: selectedAddress?.courierCartPickupAddressId,
...
export const sellerConfirmReturnApproval = async (req: Request, res: Response) => {
  try {
    ...
    const shipmentPayload = {
      ...
      original_order_id:
        shipmentPlain?.courierCart?.order_id ||
        shipmentPlain?.courierCart?.order_number ||
        String(orderDoc._id),
      original_order_number:
        shipmentPlain?.courierCart?.order_number || orderDoc.orderNumber || String(orderDoc._id),
      payment_type: 'prepaid' as const, // Returns are prepaid
      order_amount: 0, // Return shipments have no order amount
