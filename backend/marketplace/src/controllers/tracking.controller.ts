  ...
  try {
    ...
    if (!order) {
      order = await Order.findOne({
        $or: [
          ...
          { 'sellerShipments.kourierBoyzLogistics.awb_number': identifier },
        ],
      })
        .populate('user', 'name email phone')
        .lean()
    ...
    const shipment =
      (order.sellerShipments || []).find(
        (s: any) => s.shippingMeta?.awb === identifier || s.kourierBoyzLogistics?.awb_number === identifier,
    ...
    const response: any = {
      ...
      data: {
        ...
        kourierBoyzLogistics: providerTracking?.data || null,
