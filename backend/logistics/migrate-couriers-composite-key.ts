...
async function runMigration() {
  try {
    ...
    console.log('Step 1: Dropping FK constraint from kourier_boyz_zones...')
    await db.execute(sql`
      ALTER TABLE "kourier_boyz_zones"
      DROP CONSTRAINT IF EXISTS "kourier_boyz_zones_courier_id_couriers_id_fk"
    `)
    console.log('  ✓ Done\n')
