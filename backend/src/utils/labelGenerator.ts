...
export const generateLabel = async (data: LabelData): Promise<Buffer> => {
  ...
  const awbNumber = shipment.courierCart?.awb_number || shipment.shippingMeta?.awb || ''
  if (!awbNumber) {
    console.error('[generateLabel] AWB number is missing! Cannot generate AWB barcode.', {
      shipmentId: shipment._id || 'no-id',
      hasCourierCart: !!shipment.courierCart,
      hasShippingMeta: !!shipment.shippingMeta,
      courierCartAwb: shipment.courierCart?.awb_number,
  ...
  const courierName =
    ...
    (shipment.courierCart as any)?.courier_partner ||
    'Assigned Courier'
