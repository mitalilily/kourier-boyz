...
const toAdminOrderResponse = (order: any) => {
  ...
  const sellerBreakdown = (plain.sellerShipments || []).map((shipment: IOrderSellerShipment) => {
    ...
    return {
      ...
      // Explicitly include shippingMeta and kourierBoyzLogistics to ensure they're in the response
      shippingMeta: shipment.shippingMeta || null,
      kourierBoyzLogistics: shipment.kourierBoyzLogistics || null,
  ...
  const estimatedDeliveryDate =
    ...
    plain.sellerShipments?.[0]?.kourierBoyzLogistics?.estimated_delivery_date
...
export const regenerateSellerShipmentLabel = async (req: Request, res: Response) => {
  try {
    ...
    const label = await shippingProviderService.getLabel(shipment.kourierBoyzLogistics.order_id)
