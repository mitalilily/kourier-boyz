...
export const getCourierChargesReport = async (req: Request, res: Response) => {
  try {
    ...
    const shipmentQuery: any = {
      'courierCart.awb_number': { $exists: true, $ne: null },
      createdAt: { $gte: from, $lte: to },
    }
    if (seller) {
    ...
    if (awb) {
      shipmentQuery['courierCart.awb_number'] = awb
    }
    ...
    for (const shipment of shipmentRecords) {
      const awbNum = shipment.courierCart?.awb_number
      if (awbNum) {
    ...
    if (!shipmentType || shipmentType === 'Forward' || shipmentType === 'RTO') {
      ...
      if (awb) {
        orderQuery.$and[0].$or = [
          ...
          { 'sellerShipments.courierCart.awb_number': awb },
        ]
      }
      ...
      for (const order of orders) {
        for (const shipment of order.sellerShipments || []) {
          const awbNumber = shipment.shippingMeta?.awb || shipment.courierCart?.awb_number

          if (!awbNumber) continue

          // Filter by courier partner if specified
          const courier =
            shipment.shippingMeta?.courier || shipment.courierCart?.courier_id?.toString()
      ...
      for (const [awbNumber, group] of awbGroups.entries()) {
        ...
        const totalShipmentCharge =
          shipmentRecord?.courierCart?.rate ||
          firstShipment.courierCart?.rate ||
        ...
        for (const { order, shipment } of group) {
          ...
          rows.push({
            ...
            courierPartner:
              ...
              shipment.courierCart?.courier_id?.toString() ||
              'Unknown',
  ...
  try {
    ...
    for (const order of orders) {
      ...
      for (const sellerShipment of order.sellerShipments || []) {
        ...
        rows.push({
          ...tatInfo,
          courier: sellerShipment.shippingMeta?.courier || sellerShipment.courierCart?.order_id,
