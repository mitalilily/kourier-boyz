...
const OrderDetail = () => {
  ...
  const batchShipmentGroups = useMemo(() => {
    ...
    apiGroups.forEach((sg, idx) => {
      ...
      groups.push({
        key: sg.courierCartOrderId || sg.awb || sg.shipmentId || `shipment-${idx}`,
        shipment: {
  ...
  const canCancelOrder = (order: SellerOrder) => {
    ...
    const awbAlreadyGenerated =
      !!order.sellerShipment?.shippingMeta?.awb || !!order.sellerShipment?.courierCart?.awb_number
                  ...
                  <>
                    ...
                    {shipment && (
                      <>
                        ...
                        {/* Prefer AWB from shipping meta, fall back to courierCart awb_number */}
                        {(shipment.shippingMeta?.awb || shipment.courierCart?.awb_number) && (
                          <Tag color="purple">
                            AWB: {shipment.shippingMeta?.awb || shipment.courierCart?.awb_number}
                          </Tag>
                        )}
          ...
          extra={
            shipment && (
              <Space wrap>
                ...
                {/* Enable tracking as soon as we have any AWB (from shippingMeta or courierCart) */}
                {(shipment.shippingMeta?.awb || shipment.courierCart?.awb_number) && (
                  <Button
                    size="small"
                    type="primary"
                    ...
                    <Col xs={24} md={12}>
                      <Space direction="vertical" size="small" className="w-full">
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <Text strong>
                            {s.shippingMeta?.awb || s.courierCart?.awb_number || 'NA'}
                          </Text>
                        </div>
                        {(s.shippingMeta?.tracking_link ||
                          s.courierCart?.tracking_link ||
                          s.shareableTrackingLink) && (
                          <div className="flex flex-col gap-2">
                            {(s.shippingMeta?.tracking_link || s.courierCart?.tracking_link) && (
                              <div className="flex items-center justify-between">
