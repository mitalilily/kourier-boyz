    ...
    sellerShipment: sellerShipment
      ? {
          ...
          shareableTrackingLink: (() => {
            const awb = sellerShipment.shippingMeta?.awb || sellerShipment.courierCart?.awb_number
...
const buildShippingMeta = (
  ...
  return {
    awb: sellerShipment?.courierCart?.awb_number,
    courier: sellerShipment?.courierCart?.courier_id?.toString(),
    label: sellerShipment?.courierCart?.label_url,
    tracking_link: sellerShipment?.courierCart?.tracking_link,
  ...
  try {
    ...
    const allBatches: SellerOrderBatch[] = Array.from(batchMap.values()).map((batchValue) => {
      ...
      batchValue.orders.forEach((orderForBatch) => {
        ...
        const courierCartOrderId = (s.courierCart as any)?.order_id?.toString?.()
        const awb = s.shippingMeta?.awb || (s.courierCart as any)?.awb_number
        const shipmentKey = courierCartOrderId || awb || s._id.toString()
...
export const getSellerBatchShipments = async (req: Request, res: Response) => {
  try {
    ...
    orders.forEach((orderDoc) => {
      ...
      const courierCartOrderId = (sellerShipment.courierCart as any)?.order_id?.toString?.()
      const awb =
        sellerShipment.shippingMeta?.awb || (sellerShipment.courierCart as any)?.awb_number
      const shipmentKey =
        courierCartOrderId || awb || (sellerShipment._id as any)?.toString?.() || orderId
...
export const createSellerShipment = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (sellerShipment.courierCart?.order_id) {
      return res.json({
        success: true,
    ...
    if (
      sellerShipment.courierCart?.order_number === shipmentPayload.order_number &&
      sellerShipment.courierCart?.order_id &&
      sellerShipment.courierCart?.awb_number
    ) {
      console.info('[Shipmozo] Reusing existing shipment from previous attempt', {
        ...courierLogContext,
        courierCartOrderId: sellerShipment.courierCart.order_id,
        awb: sellerShipment.courierCart.awb_number,
      })
      shipmentResponse = {
        success: true,
        data: {
          order_id: sellerShipment.courierCart.order_id,
          order_number: sellerShipment.courierCart.order_number || shipmentPayload.order_number,
          awb_number: sellerShipment.courierCart.awb_number,
          status: 'booked',
          label: sellerShipment.courierCart.label_url,
...
export const requestPickup = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Also allow if shipment already has courierCart data (already processed but may need labels/invoices)
    if (
    ...
    // Track fragile flags by courierCart order_id for label generation
    const fragileFlagsByCourierCartOrderId = new Map<string, boolean>()
    ...
    for (const shipmentInput of shipmentInputs) {
      ...
      const existingShipment = groupShipments.find(
        (gs) =>
          gs.courierCart?.order_number === compactOrderNumber &&
          gs.courierCart?.order_id &&
          gs.courierCart?.awb_number,
      )

      let shipmentResponse: Awaited<ReturnType<typeof shippingProviderService.createShipment>>
      if (existingShipment?.courierCart) {
        console.info('[Shipmozo] Reusing existing shipment from previous attempt', {
          ...courierLogContext,
          courierCartOrderId: existingShipment.courierCart.order_id,
          awb: existingShipment.courierCart.awb_number,
        })
        shipmentResponse = {
          success: true,
          data: {
            order_id: existingShipment.courierCart.order_id!,
            order_number: existingShipment.courierCart.order_number || compactOrderNumber,
            awb_number: existingShipment.courierCart.awb_number!,
            status: 'booked',
            label: existingShipment.courierCart.label_url,
            courier_partner: existingShipment.shippingMeta?.courier,
            tracking_link: existingShipment.courierCart.tracking_link,
            createManifest: true,
          },
        }
      } else {
      ...
      // Track fragile flag by courierCart order_id for label generation
      if (shipmentData?.order_id) {
        fragileFlagsByCourierCartOrderId.set(shipmentData.order_id, fragileValue)
      }
    ...
    // Persist updated shipment details (courierCart, shippingMeta, package) for all grouped orders
    groupOrders.forEach((orderDoc) => orderDoc.markModified('sellerShipments'))
    await Promise.all(groupOrders.map((orderDoc) => orderDoc.save()))
    ...
    if (manifestData && manifestData.length > 0) {
      ...
      // Attach manifest only to shipments that were actually updated (have courierCart data)
      groupShipments.forEach((groupShipment, index) => {
        if (updatedOrderIndexes.has(index) && groupShipment.courierCart?.order_id) {
    ...
    // Group orders by courierCart.order_id to identify shipment groups
    // (fragileFlagsByCourierCartOrderId is already declared above)
    ...
    for (let orderIndex = 0; orderIndex < groupOrders.length; orderIndex++) {
      ...
      if (!shipmentForOrder?.courierCart?.order_id) continue

      const courierCartOrderId = shipmentForOrder.courierCart.order_id
      if (!shipmentLabelGroups.has(courierCartOrderId)) {
        shipmentLabelGroups.set(courierCartOrderId, [])
      }
      shipmentLabelGroups.get(courierCartOrderId)!.push(groupOrder)
    }

    // Generate ONE label per shipment group with all items from all orders in that shipment
    for (const [courierCartOrderId, ordersInShipment] of shipmentLabelGroups.entries()) {
      ...
      const shipmentForFirstOrder = populatedFirstOrder.sellerShipments.find(
        (s) =>
          s?.seller?._id?.toString() === sellerId && s.courierCart?.order_id === courierCartOrderId,
      )
      ...
      const awbNumber =
        shipmentForFirstOrder.courierCart?.awb_number || shipmentForFirstOrder.shippingMeta?.awb
      if (!awbNumber) {
        console.log(
          `[requestPickup] AWB not available yet for courierCart order ${courierCartOrderId}, skipping label generation`,
        )
        continue
      }
      ...
      console.log('[requestPickup] Shipment for label generation:', {
        ...
        courierCartAwb: shipmentForFirstOrder.courierCart?.awb_number,
      ...
      if (shipmentForFirstOrder.label?.label_url) {
        console.log(
          `[requestPickup] Shipment label already exists for courierCart order ${courierCartOrderId}`,
        )
        continue
      }
        ...
        if (populatedOrderInShipment) {
          ...
          const shipmentInGroup = populatedOrderInShipment.sellerShipments.find(
            (s) =>
              ...
              s.courierCart?.order_id === courierCartOrderId,
          )
      ...
      const trackedFragileFlag = fragileFlagsByCourierCartOrderId.get(courierCartOrderId)
      const fragileFlag =
      ...
      const combinedShipment = {
        ...
        courierCart: {
          ...shipmentForFirstOrder.courierCart,
          awb_number: shipmentForFirstOrder.courierCart?.awb_number || awbNumber,
        },
        shippingMeta: {
      ...
      console.log('[requestPickup] Combined shipment for label:', {
        ...
        awbNumber: combinedShipment.courierCart?.awb_number || combinedShipment.shippingMeta?.awb,
        hasCourierCart: !!combinedShipment.courierCart,
      ...
      console.log(
        `[requestPickup] Generated ONE shipment label for ${ordersInShipment.length} orders shipped together (courierCart order: ${courierCartOrderId}) with ${allShipmentItemsForLabel.length} items`,
      )
    }
    ...
    try {
      for (let orderIndex = 0; orderIndex < groupOrders.length; orderIndex++) {
        ...
        if (populatedOrder) {
          ...
          // Only generate invoices if this shipment has courierCart data
          // (meaning it was actually part of a shipment)
          if (!shipmentForOrder.courierCart?.order_id) {
            console.log(
          ...
          if (!shipmentForOrder.invoice?.invoice_url) {
            try {
              ...
              const courierCharges =
                ...
                shipmentForOrder.courierCart?.rate ??
          ...
          try {
            ...
            const awb =
              shipmentForOrder.shippingMeta?.awb || shipmentForOrder.courierCart?.awb_number
            const trackingLink =
              shipmentForOrder.shippingMeta?.tracking_link ||
              shipmentForOrder.courierCart?.tracking_link ||
              undefined
            const labelUrl =
              shipmentForOrder.label?.label_url ||
              shipmentForOrder.shippingMeta?.label ||
              shipmentForOrder.courierCart?.label_url
      ...
      for (const [index, groupOrder] of groupOrders.entries()) {
        if (updatedOrderIndexes.has(index)) {
          ...
          // Only update if shipment has courierCart data (was part of a shipment)
...
export const getSellerShipmentLabel = async (req: Request, res: Response) => {
  try {
    ...
    const label = await shippingProviderService.getLabel(sellerShipment.courierCart.order_id)
...
export const downloadSellerInvoice = async (req: Request, res: Response) => {
  try {
    ...
    const courierCharges =
      ...
      sellerShipment.courierCart?.rate ??
