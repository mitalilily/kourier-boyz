...
const Footer = () => {
  ...
  return (
    <footer className="kb-footer-surface relative z-10 text-[#252726]">
      <div className="kb-footer-cta border-b border-[#8b681e]/20">
        <div className="mx-auto flex max-w-7xl flex-col gap-6 px-5 py-10 sm:px-8 md:flex-row md:items-center md:justify-between lg:px-10">
          <div>
            <p className="text-xs font-semibold uppercase text-[#66490e]">One useful platform, whichever way you begin</p>
          ...
          <div className="flex shrink-0 flex-wrap gap-3">
            <Link to="/shop" className="kb-button bg-white text-[#1d1d1c] shadow-sm">Visit marketplace <ArrowRight className="h-4 w-4" /></Link>
            <Link to="/ship" className="kb-button border border-[#725313]/35 bg-white/35 text-[#30250c]">Start shipping <Send className="h-4 w-4" /></Link>
          </div>
        </div>
      </div>

      <div className="mx-auto max-w-7xl px-5 sm:px-8 lg:px-10">
        <div className="grid gap-12 border-b border-black/10 py-14 lg:grid-cols-[1.1fr_1.4fr] lg:py-18">
          <div>
            ...
            <p className="mt-6 max-w-md text-base leading-7 text-[#555b58]">
            ...
            <div className="mt-7 grid gap-3 text-sm text-[#454b48]">
          ...
          <div className="grid grid-cols-2 gap-x-6 gap-y-10 sm:grid-cols-3">
            {navigation.map((section) => (
              <nav key={section.title} aria-label={section.title}>
                <h3 className="text-sm font-semibold text-[#906510]">{section.title}</h3>
        ...
        <div className="grid gap-8 border-b border-black/10 py-9 md:grid-cols-[1fr_auto] md:items-center">
          <div>
            <h3 className="text-xl font-semibold">Useful updates, not inbox clutter.</h3>
            <p className="mt-2 text-sm text-[#5f6662]">Product finds, seller tools, and practical shipping updates.</p>
          </div>
          <form onSubmit={submitNewsletter} className="flex w-full max-w-xl flex-col gap-2 sm:flex-row md:w-[480px]">
            <label className="sr-only" htmlFor="footer-email">Email address</label>
            <input id="footer-email" type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="Email address" className="h-12 min-w-0 flex-1 border border-black/15 bg-white/80 px-4 text-[#252726] outline-none placeholder:text-[#777d79] focus:border-[#b78115]" />
        ...
        <div className="flex flex-col gap-5 py-6 text-xs text-[#646a67] sm:flex-row sm:items-center sm:justify-between">
          <p>© {new Date().getFullYear()} Kourier Boyz. All rights reserved.</p>
          <div className="flex flex-wrap items-center gap-5">
            <Link to="/privacy-policy" className="hover:text-[#8c6210]">Privacy</Link>
            <Link to="/terms" className="hover:text-[#8c6210]">Terms</Link>
            <Link to="/return-refund-policy" className="hover:text-[#8c6210]">Returns</Link>
            {settings?.socialLinks?.map((social) => {
              const Icon = socialIcons[social.platform as keyof typeof socialIcons]
              return Icon ? <a key={social.platform} href={social.url} target="_blank" rel="noreferrer" aria-label={social.platform} className="text-[#626865] transition hover:-translate-y-0.5 hover:text-[#9a6b10]"><Icon className="h-4 w-4" /></a> : null
            })}
          </div>
        </div>
      </div>
