...
const resolveCourierProvider = (requestPath = ''): CourierProvider =>
  requestPath.includes('/shipmozo') ? 'shipmozo' : 'kourier-boyz-logistics'
...
export const handleKourierBoyzLogisticsWebhook = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (courierOrderId) {
      filters.push({ 'sellerShipments.kourierBoyzLogistics.order_id': courierOrderId })
    }
    if (awb) {
    ...
    const shipment =
      order.sellerShipments.find(
        (sellerShipment) =>
          (courierOrderId && sellerShipment.kourierBoyzLogistics?.order_id === courierOrderId) ||
    ...
    try {
      ...
      const trackingLink =
        shipment.shippingMeta?.tracking_link || shipment.kourierBoyzLogistics?.tracking_link || undefined
        ...
        case 'in_transit': {
          ...
          if (buyerPhone) {
            ...
            const awb =
              ...
              (shipment as any)?.kourierBoyzLogistics?.awb ||
              (shipment as any)?.shippingMeta?.tracking_id ||
              (shipment as any)?.kourierBoyzLogistics?.tracking_id ||
              trackingLink ||
              ''
...
export const handleShipmozoWebhook = handleKourierBoyzLogisticsWebhook
