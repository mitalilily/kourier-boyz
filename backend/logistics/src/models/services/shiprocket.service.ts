...
const buildAmazonAddressFromLooseInput = async ({
  ...
  return buildAmazonShippingAddressFromWarehouse({
    alias: trimText(name) || trimText(companyName) || 'Kourier Boyz',
    contactName: trimText(name) || trimText(companyName) || 'Kourier Boyz',
    ...
    companyName: trimText(companyName) || trimText(name) || 'Kourier Boyz',
  })
}
  ...
  if (userId && pickupId) {
    ...
    if (pickupWarehouse && !isReverseShipment) {
      shipFrom = buildAmazonShippingAddressFromWarehouse({
        ...
        companyName: pickupWarehouse.addressNickname || pickupWarehouse.contactName || 'Kourier Boyz',
      })
    }
  }
...
function buildPickupFromWarehouse(
  ...
  return {
    ...
    name: warehouse.contactName || 'Kourier Boyz',
...
export const createB2CShipmentService = async (
  ...
  try {
    ...
    params.company = {
      ...(params.company || {}),
      name: resolvedCompanyName || 'Kourier Boyz',
  ...
  if (!Number.isFinite(freightCharges) || freightCharges <= 0) {
    throw new HttpError(
      ...
      isReverseShipment
        ...
        : 'No Kourier Boyz rate card freight available for selected courier/zone',
    )
  }
  ...
  try {
    ...
    const result: any = await db.transaction(async (tx) => {
      ...
      if (isReverseShipment) {
        ...
        // Seller wallet debited: freight_charges (Kourier Boyz rate-card freight) + other_charges (fuel surcharge, handling, etc.)
        applyConfiguredGstToWalletDebit(freightCharges + otherCharges)
        ...
        // Seller wallet debited: freight_charges (Kourier Boyz rate-card freight) + other_charges (fuel surcharge, handling, etc.) + cod_charges (courier COD fee)
      ...
      // Download the Kourier Boyz label URL and save to R2, or generate a platform label.
      if (
        params.integration_type === 'kourier-boyz' ||
        params.integration_type === 'kourier-boyz-logistics' ||
        ...
        try {
          ...
          if (freshOrder) {
            // Try to download the Kourier Boyz label URL first if available.
            const kourierBoyzLogisticsLabelUrl = shipmentMeta?.label

            if (
              kourierBoyzLogisticsLabelUrl &&
              typeof kourierBoyzLogisticsLabelUrl === 'string' &&
              kourierBoyzLogisticsLabelUrl.startsWith('http')
            ) {
              try {
                console.log(`Downloading Kourier Boyz label from URL: ${kourierBoyzLogisticsLabelUrl}`)

                // Download label PDF from the platform URL.
                const labelResponse = await axios.get(kourierBoyzLogisticsLabelUrl, {
                ...
                console.log(`Kourier Boyz label downloaded and saved to R2: ${labelKey}`)
              } catch (downloadErr: any) {
                console.error(
                  `Failed to download Kourier Boyz label from URL: ${kourierBoyzLogisticsLabelUrl}`,
                ...
                if (labelKey) {
                  await tx
                    ...
                    .where(eq(b2c_orders.id, newOrder.id))
                  console.log(`Kourier Boyz custom label generated and saved: ${labelKey}`)
                }
              }
            } else {
              // No hosted platform label URL, generate a custom Kourier Boyz label.
              console.log(
                `No Kourier Boyz label URL, generating custom label for ${params.order_number}`,
              )
              const labelKey = await generateLabelForOrder(freshOrder, userId, tx)
              if (labelKey) {
                await tx
                ...
                console.warn(
                  `Kourier Boyz label generator returned empty result for ${params.order_number}`,
                )
              }
            }
          }
          ...
          console.error(
            `Failed to process Kourier Boyz label for ${params.order_number}:`,
...
export const createB2BShipmentService = async (
  ...
  if (effectiveIntegrationType === 'delhivery') {
    ...
    try {
      ...
      const pickupContactName = String(payload.pickup?.name || '').trim() || 'Kourier Boyz'
      ...
      const billingAddress: Record<string, any> = {
        ...
        company: String(payload.company?.name || payload.pickup?.warehouse_name || 'Kourier Boyz').trim(),
        consignor: String(payload.company?.name || payload.pickup?.warehouse_name || 'Kourier Boyz').trim(),
        address: pickupAddress,
        city: pickupCity,
        state: pickupState,
        pin: pickupPincode,
    ...
    try {
      ...
      shipmentRequest = {
        ...
        consignor: {
          ...
          email: cleanApptmyzText(resolvedPickupWarehouse?.contactEmail || 'support@kourierboyz.com'),
        },
        consignee: {
          consigneeCode:
            cleanApptmyzText(
