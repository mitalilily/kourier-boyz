...
export const publicCheckServiceability = async (req: Request, res: Response) => {
  try {
    ...
    const product = await Product.findById(productId).populate(
      'seller',
      'courierCartPickupAddressId pickupAddresses addressLine1 city state postalCode country',
    )
    ...
    // Priority: 1. Query param pickup_id/origin, 2. pickupAddresses (default or first), 3. root courierCartPickupAddressId, 4. business address postalCode
    ...
    if (!pickupId && !originPincode) {
      ...
      if (
        ...
        if (defaultPickup?.courierCartPickupAddressId) {
          pickupId = defaultPickup.courierCartPickupAddressId
        }
      ...
      // Fallback to root-level courierCartPickupAddressId
      if (!pickupId && !originPincode && seller?.courierCartPickupAddressId) {
        pickupId = seller.courierCartPickupAddressId
      }
