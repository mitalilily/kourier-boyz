...
export const notifyOrderShipped = async (
  ...
  try {
    ...
    const trackingLink =
      ...
      sellerShipment.courierCart?.tracking_link ||
      undefined

    // Get AWB and courier info
    const awb =
      sellerShipment.shippingMeta?.awb || sellerShipment.courierCart?.awb_number || undefined
    const courier = sellerShipment.shippingMeta?.courier || undefined
    const estimatedDelivery =
      sellerShipment.shippingMeta?.estimated_delivery_date ||
      sellerShipment.courierCart?.estimated_delivery_date ||
      undefined
    ...
    const effectiveTrackingLink = trackingIdentifier
      ...
      : trackingLink // Fallback to CourierCart link if no identifier available
...
export const notifyOrderDelivered = async (order: HydratedDocument<IOrder>) => {
  try {
    ...
    const trackingLink =
      ...
      firstShipment?.courierCart?.tracking_link ||
      undefined
